...
import { B as Block, S as Static, e as BlockLabel, C as Chat, h as IconButtonWrapper, i as IconButton, G as Community, H as Trash, J as MarkdownCode, K as Copy, N as ScrollDownArrow, R as Retry, v as Undo } from './2-9Q2E4iJ-.js';
...
const ChatBot = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  })} ${pending_message ? `${validate_component(Pending, "Pending").$$render($$result, { layout, avatar_images }, {}, {})}` : `${options ? `<div class="options svelte-lykdgn">${each(options, (option, index) => {
    return `<button class="option svelte-lykdgn">${escape(option.label || option.value)} </button>`;
  })}</div>` : ``}`}</div>` : `<div class="placeholder-content svelte-lykdgn">${placeholder !== null ? `<div class="placeholder svelte-lykdgn">${validate_component(MarkdownCode, "Markdown").$$render(
    $$result,
    {
      message: placeholder,
      latex_delimiters,
...
//# sourceMappingURL=Index56-JRyMk0x7.js.map
